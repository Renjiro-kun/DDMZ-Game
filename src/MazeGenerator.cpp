...
void MazeGenerator::GenerateMesh(MazeGenerator::MazeInfo& info, Mesh& outMesh)
{
    ...
    for (size_t z = 0; z < info.height; z++)
    {
        for (size_t x = 0; x < info.width; x++)
        {
            ...
            if(((char*)layerInfo->data)[z*info.width + x] == wallID)
            {
                ...
                if(((z < info.height - 1) && ((char*)layerInfo->data)[(z+1)*info.width+x] == 0) || (z == info.height - 1))
                {
                ...
                if(((z > 0) && ((char*)layerInfo->data)[(z-1)*info.width+x]) || (z==0))
                {
                ...
                if(((x < info.width - 1) && ((char*)layerInfo->data)[z*info.width + (x+1)] == 0) || (x==info.width-1))
                {
                ...
                if(((x > 0) && ((char*)layerInfo->data)[z*info.width + (x-1)] == 0) || (x==0))
                {
                    ...
                    mapTexcoord[tcCounter + 5] = (Vector2){leftTexUV.x + leftTexUV.width, leftTexUV.y + leftTexUV
                    .height};
                    tcCounter += 6;
            ...
            else if(((char*)layerInfo->data)[z*info.width + x] == 0)
            {
                // Define top triangles (2 tris, 6 verts --> v1-v2-v3, v1-v3-v4)
                mapVerticies[vCounter] = v1;
